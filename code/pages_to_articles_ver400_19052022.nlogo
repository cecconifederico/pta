...
globals [articles_db pages_db websites_db users_db actual_page_list actual_article_pool actual_article_web_pool issue_w intensity_w political_direction id_sim_article parameters]

to import_world
  clear-all
  import-world "pta_base.txt"
end

to-report choose_pages_by_pageProperties [actual_page]
  if pages_schema = 1 [
    let actual_rank 0
    let x [followers] of actual_page
    if is-number? x [
      if x < 1 [set x 1]
      let x_rank log x 10
      let y count link-neighbors
      if y < 1 [set y 1]
      let y_rank log y 10
      set actual_rank x_rank + y_rank
    ]
    report (list actual_page actual_rank)
  ]
end

to create_articles_pool
  set issue_w []
  foreach  parameters [ i ->
    let title (item 0 i)
    let iss   (item 1 i)
    let val   (item 2 i)
    if article_issue_schema = title
     [
        set issue_w lput (list iss val) issue_w
     ]
  ]
...
let article_in_pool 0
let article_round 0
while [article_round < 10 and article_in_pool < 15][
    set article_round article_round + 1
    let my_article one-of articles
    let article_rank 0
    let max_rank 1
    let value_list []
    if article_contents_weigth = true [
      set max_rank max_rank + 5
      if is-list? issue_w [
      set value_list filter [ i -> (item 0 i) = [issue] of my_article] issue_w
      if (length value_list) > 0 [
       let x (item 0 value_list)
       set article_rank article_rank + (item 1 x)
       ]
      ]
    ]

    if article_intensity_weigth = true [
      set max_rank max_rank + 15
      if is-list? intensity_w [
      set value_list filter [ i -> (item 0 i) = [intensity] of my_article] intensity_w
      if (length value_list) > 0 [
       let x (item 0 value_list)
       set article_rank article_rank + (item 1 x)
      ]
    ...
    if [political] of my_article = 1 and article_political_schema != "neutral" [
      set max_rank max_rank + 1
      if article_political_schema  = "1 R" and [political_orientation] of my_article = "destra" [set article_rank article_rank + 1]
      if article_political_schema  = "-1 R" and [political_orientation] of my_article = "destra" [set article_rank article_rank - 1]
      if article_political_schema  = "1 L" and [political_orientation] of my_article = "sinistra" [set article_rank article_rank + 1]
      if article_political_schema  = "-1 L" and [political_orientation] of my_article = "sinistra" [set article_rank article_rank - 1]
    ]

   set max_rank max_rank + article_incitement_schema * 2
   let a [incitement_to_hate] of my_article
   let b [incitement_to_fear] of my_article
   set article_rank  article_rank + a * article_incitement_schema + b * article_incitement_schema

   let tot_rank article_rank / max_rank
   if tot_rank > 1 [set tot_rank 1]
   if tot_rank < 0 [set tot_rank 0]

   if (random-float 1) < tot_rank [
      set actual_article_pool lput  my_article actual_article_pool
   ]
]
...
to run_simulation
  ...
  set actual_page_list []
  let page_pool n-of 10 pages
  ask page_pool [
      set actual_page_list lput (choose_pages_by_pageProperties self) actual_page_list
  ]
  let actual_page first rnd:weighted-one-of-list actual_page_list [ [p] -> last p ]

  set actual_article_pool []
  create_articles_pool

  set actual_article_web_pool []
  foreach actual_article_pool [ the-turtle ->
  ask the-turtle [
    ask link-neighbors [
        if is-website? self [
        let a fi / 30
        let b ex / 5
        let c sum_of_interactions / 2000
        if a > 1 [set a 1]
        if b > 1 [set b 1]
        if c > 1 [set c 1]
        let d (a + b + c) / 3
        set actual_article_web_pool lput (list the-turtle d) actual_article_web_pool
        ]
     ]
   ]
  ]

  let the_winner_is nobody


  foreach actual_article_web_pool [the-article ->

    if (item 1 the-article) > (random-float 1) [
      ; show (item 0 the-article)
      ask actual_page [
        set color red
        ask (item 0 the-article) [set color red]
        set id_sim_article lput  (list ([id_page] of actual_page) ([id_article] of (item 0 the-article))) id_sim_article
      ]
    ]
  ]
 tick
...
to setup_simulation
  ...
  set parameters (csv:from-file "parameter.csv" ";")

  set pages_db but-first pages_db
  foreach pages_db [x ->
    let idp (item 0 x)
    let listidp [id_page] of pages
    if (member? idp listidp) = false [
    create-pages 1[
  ...
  foreach users_db [x ->
    create-users 1[
      ...
      create-usetopag-with mypage [ifelse random-float 1 < 0.9 [set hidden? true][set hidden? false]]
      set shape "person"
  ...
  foreach articles_db [x ->
    create-articles 1[
      ...
      create-arttoweb-with mywebsite [ifelse random-float 1 < 0.9 [set hidden? true][set hidden? false]]
      set shape "square"
...
article_issue_schema
"default" "Covid Cospirationism" "NWO" "Traditional Cospirationism" "Health" "Environment and Climate"
5
...
@#$#@#$#@
<experiments>
  <experiment name="experiment_1000" repetitions="1" runMetricsEveryStep="false">
    <setup>setup_simulation</setup>
    <go>run_simulation</go>
    <timeLimit steps="1000"/>
    <metric>id_sim_article</metric>
    <enumeratedValueSet variable="article_political_schema">
      <value value="&quot;neutral&quot;"/>
      <value value="&quot;1 L&quot;"/>
      <value value="&quot;1 R&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="article_intensity_weigth">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="pages_schema">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="article_contents_weigth">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="websites_schema">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="article_incitement_schema">
      <value value="1"/>
      <value value="3"/>
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="article_issue_schema">
      <value value="&quot;default&quot;"/>
      <value value="&quot;political&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Scenarios">
      <value value="&quot;From interface&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="modality">
      <value value="&quot;Random setup&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment_multi_issue_19052022" repetitions="10" runMetricsEveryStep="false">
    <setup>setup_simulation</setup>
    <go>run_simulation</go>
    <timeLimit steps="1000"/>
    <metric>id_sim_article</metric>
    ...
    <enumeratedValueSet variable="article_issue_schema">
      <value value="&quot;default&quot;"/>
      <value value="&quot;Covid Cospirationism&quot;"/>
      <value value="&quot;NWO&quot;"/>
      <value value="&quot;Traditional Cospirationism&quot;"/>
      <value value="&quot;Health&quot;"/>
      <value value="&quot;Environment and Climate&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Scenarios">
      <value value="&quot;From interface&quot;"/>
    </enumeratedValueSet>
